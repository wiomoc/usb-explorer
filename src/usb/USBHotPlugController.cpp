...
int USBHotPlugController::usb_hotplug_callback_fn(libusb_context *ctx, libusb_device *device, libusb_hotplug_event event, void *user_data)
{
    ...
    std::weak_ptr<USBHotPlugController> weak_controller = controller->weak_self;
...
{
    USBHotPlugController *controller = new USBHotPlugController(usb_ctx, event_handler, callback);
    std::shared_ptr<USBHotPlugController> controller_shared(controller);
    controller->weak_self = controller_shared;
    ...
    new std::thread([=] {
        while (!controller_shared->stopped)
            libusb_handle_events(usb_ctx);
    });

    return controller_shared;
}
